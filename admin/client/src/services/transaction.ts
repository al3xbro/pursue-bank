async function internalTransaction(toEmail: string, amt: string) {
  const res = await fetch(`${import.meta.env.VITE_USER_SERVER_URL}/api/internal/transaction`, {
    method: 'POST',
    headers: {
...
async function internalRecurring(toEmail: string, amt: string, day: string) {
  const res = await fetch(`${import.meta.env.VITE_USER_SERVER_URL}/api/internal/autopay`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      Authorization: `Bearer ${localStorage.getItem('accessToken')}`,
...
async function getBalance() {
  const res = await fetch(`${import.meta.env.VITE_USER_SERVER_URL}/api/internal/transaction/balance`, {
    method: 'GET',
    headers: {
      Authorization: `Bearer ${localStorage.getItem('accessToken')}`,
    },
...
async function getUser() {
  const res = await fetch(`${import.meta.env.VITE_USER_SERVER_URL}/api/internal/user`, {
    method: 'GET',
    headers: {
      Authorization: `Bearer ${localStorage.getItem('accessToken')}`,
    },
